...
export class NotificationMapper {
  ...
  static toPersistence(notification: Notification) {
    ...
    entity.items = notification.items.map((item) => {
      const notificationItemEntity = new NotificationItemEntity();
      notificationItemEntity.id = item.id;
      notificationItemEntity.name = item.name;
      notificationItemEntity.type = item.type;
